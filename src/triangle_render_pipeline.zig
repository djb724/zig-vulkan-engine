...
pub const PipelineResources = struct {
    ...
    pub fn teapotPipeline(allocator: Allocator, io: std.Io, device: Device, extent: vk.Extent2D, format: vk.Format) !PipelineResources {
        const pipelines = try allocator.alloc(vk.Pipeline, 1);
        defer allocator.free(pipelines);
        ...
        const vert_shader = try shaders.loadShader(io, device, "teapot.vert.spv");
        defer device.destroyShaderModule(vert_shader, null);

        const frag_shader = try shaders.loadShader(io, device, "teapot.frag.spv");
        defer device.destroyShaderModule(frag_shader, null);
